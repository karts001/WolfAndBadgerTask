...
class TestTheUpdateInfoRoute(unittest.TestCase):
    ...
    def test_the_update_info_route_has_flask_form_with_two_buttons(self):
        ...
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value="karts001"):
    ...
    def test_the_form_redirects_to_success_route_when_validated_data_is_sent(self):
        ...
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value=mock_github_user):

            response = self.client.post('/authorized/update_info', data=dict(
            ...
            exists = db.session.query(PersonalInfo).filter_by(github_user_name=mock_github_user).first() is not None
            self.assertEqual(response.status_code, 200)
    ...
    def test_the_form_does_not_validate_if_the_age_field_contains_a_value_not_in_the_range_0_100(self):
        mock_github_user = 'test'
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value=mock_github_user):
    ...
    def test_the_form_does_not_validate_if_the_email_field_contains_an_invalid_email_address(self):
        mock_github_user = 'test'
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value=mock_github_user):
    ...
    def test_the_form_does_not_validate_if_the_phone_number_field_does_not_contian_between_12_and_16_characters(self):
        mock_github_user = 'test'
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value=mock_github_user):
    ...
    def test_the_form_is_populated_with_the_data_from_the_database_if_a_user_exists(self):
        mock_github_user = 'test'
        # personal_info_form = PersonalInfoForm()
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value=mock_github_user):
    ...
    def test_the_delete_record_button_removes_the_users_data_from_the_database(self):
        ...
        with mock.patch('flask_app.src.routes.views.get_user_info', return_value=mock_github_user):
            ...
            exists = db.session.query(PersonalInfo).filter_by(github_user_name=mock_github_user).first()
            ...
            self.assertEqual(1, len(response.history))
            self.assertEqual(f'/authorized/delete_record/{mock_github_user}', response.request.path)
